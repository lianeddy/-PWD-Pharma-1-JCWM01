...
module.exports = {
    ...
    getDataProductId: (req,res) => {
        ...
        let getQueryId = `SELECT * FROM obat WHERE id_obat=${db.escape(req.params.id)}`
    ...
    updateDataProduct: (req,res) => {
        ...
        upload(req, res, (error) => {
            ...
            if(file){
                ...
                let updateQuery = `UPDATE obat SET nama_obat = ${db.escape(namaObat)}, jumlah_obat = ${db.escape(jumlahObat)}, satuan = ${db.escape(satuan)}, deskripsi = ${db.escape(deskripsi)}, manfaat = ${db.escape(manfaat)}, komposisi = ${db.escape(komposisi)}, dosis = ${db.escape(dosis)}, golongan = ${db.escape(golongan)}, harga_pokok = ${db.escape(hargaPokok)}, harga_jual = ${db.escape(hargaJual)}, foto_obat = ${db.escape(filePath)} WHERE id_obat = ${db.escape(idObat)} `
                ...
                db.query(updateQuery, (err, result) => {
                    ...
                    let getQueryId = `SELECT * FROM obat WHERE id_obat=${db.escape(idObat)}`
                    db.query(getQueryId, (err, result) =>{
                        if(err) res.status(500).send(err)

                        res.status(200).send({result, message: "Data berhasil di perbarui", success: true})
                    })
                    
                })
            }else{

                let updateQuery = `UPDATE obat SET nama_obat = ${db.escape(namaObat)}, jumlah_obat = ${db.escape(jumlahObat)}, satuan = ${db.escape(satuan)}, deskripsi = ${db.escape(deskripsi)}, manfaat = ${db.escape(manfaat)}, komposisi = ${db.escape(komposisi)}, dosis = ${db.escape(dosis)}, golongan = ${db.escape(golongan)}, harga_pokok = ${db.escape(hargaPokok)}, harga_jual = ${db.escape(hargaJual)}, foto_obat = ${db.escape(fotoObat)} WHERE id_obat = ${db.escape(idObat)} `
